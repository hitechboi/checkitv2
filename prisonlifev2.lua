...
while not _0x11 and not _G.lib do
    ...
    waited=waited+0.1
    if waited>=5 then
        pcall(function()notify("Check it","UI library timeout",5)end)
        return
    end
end
...
local _0xTargetInitOpts = {"All Criminals"}
local _0xTargetDD = autoSec:Dropdown({label="arrest target",options=_0xTargetInitOpts,default="All Criminals",id="arrest_target",col=1,callback=function(val)
    if val == "All Criminals" then _0xArrestTarget = nil
    else
...
_G.MyMoms_Cleanup = function()
    ...
    if _0x11 and type(_0x11.Destroy) == "function" then pcall(function() _0x11:Destroy() end) end
