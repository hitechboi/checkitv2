...
function ChildVm:_startEngine()
    ...
    task.spawn(function()
        while self._running do
            local w=self._watchers; local alive={}
...
function ChildVm:OnChildAdded(parent,cb)
    local cur=snapshotchildren(parent); local pending={}
...
function ChildVm:OnChildRemoved(parent,cb)
    local cur=snapshotchildren(parent); local missing={}
    local w={active=true,poll=function()
        local now=snapshotchildren(parent)
        for a,c in pairs(cur) do
            if not now[a] then missing[a]=(missing[a]or 0)+1; if missing[a]>=2 then missing[a]=nil; cur[a]=nil; pcall(cb,c) end
...
connectsignal(runservice.Heartbeat,function()
    ...
    if not noragdoll and not noblock and not state.infinitestamina then return end
    pcall(function()
...
local function showhitmarker(dmg,pos)
    ...
    txt.Text=tostring(mfloor(dmg)); txt.Size=28; txt.Center=true; txt.Outline=true
    pcall(function() txt.OutlineColor=Color3.fromRGB(0,0,0) end)
    txt.Color=damagecolors[math.random(1,#damagecolors)]; txt.Font=3
...
local function updatehitmarkers()
    ...
    local now=os.clock(); local k=1
    while k<=#damagetexts do
        local d=damagetexts[k]; local el=now-d.t
        if el>d.d then pcall(function() d.txt:Remove() end); table.remove(damagetexts,k)
        else
            if not hitmarkersenabled then d.txt.Visible=false; k=k+1
            else
                d.p=d.p+Vector3.new(d.vx*0.05,d.vy*0.05,d.vz*0.05); d.vy=d.vy-0.2
                local sp,on; pcall(function() sp,on=WorldToScreen(d.p) end)
                if on and sp then
                    local a=1; if el>(d.d-0.5) then a=1-(el-(d.d-0.5))/0.5 end
                    d.txt.Position=Vector2.new(sp.X,sp.Y); d.txt.Transparency=a; d.txt.Visible=true
...
local function getroot(mdl,ig)
    ...
    if ig then
        local rt; pcall(function() rt=mdl:FindFirstChild("Point1") end)
...
local function getname(mdl,ig,e)
    ...
    if e and n~="" and n~="bloodsplat" and n~="BloodSplat" and n~="Model" then e._nameCache=n end
    return n,false
end
...
local function deletetrap(m)
    ...
    pcall(function()
        for _,p in ipairs(m:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false; p.Transparency=1 end end
        if m:IsA("BasePart") then m.CanCollide=false; m.Transparency=1 end
    end)
    pcall(function() m.Parent=nil end)
end

local function scanplayer(m)
    if not m then return end
    local isSelf=(m.Name==localplayer.Name or m==localplayer.Character)
    if not state.optself and isSelf then return end
    if not m:FindFirstChildOfClass("Humanoid") then return end
    local a=m.Address; if not a or state.playerdata[a] then return end
    local nm=getname(m,false,nil)
    local e=espmod.newtracker(m,nm,state.playercolor,{isSelf=isSelf})
    if e then state.playerdata[a]=e end
end

local function scankiller(m)
    if not m then return end
    if not m:FindFirstChildOfClass("Humanoid") then return end
    local a=m.Address; if not a or state.killerdata[a] then return end
    local nm=getname(m,false,nil)
    local e=espmod.newtracker(m,nm,state.killercolor,{isKiller=true})
    if e then state.killerdata[a]=e end
end

local function scangenerator(m)
    if not m then return end
    local a=m.Address; if not a or state.gendata[a] then return end
    local e=espmod.newtracker(m,"Generator",state.gencolor,{
        isObject=true, isGen=true,
        custom_update=function(t, minx, miny, maxx, maxy)
            ...
            if dn then
                ...
                t.box.Color=dg; t.namelabel.Color=dg; t.distlabel.Color=dg
            else
                t.distlabel.Text=espmod.tag_open.."prog: "..tostring(pv).."%"..espmod.tag_close
                t.distlabel.Color=gg; t.namelabel.Color=state.gencolor; t.box.Color=state.gencolor
            end
        end
    })
...
local function querygenerators()
    local f=getgenfolder(); if not f then return end
    local seen,changed,scanned_pos={},false,{}
    local function checkgen(c)
        if not c then return end
        local p=c.Parent; local isInner=false
    ...
    for _,c in ipairs(f:GetChildren()) do if c:IsA("Model") or c.Name=="Generator" then checkgen(c) end end
    pcall(function()
        for _,c in ipairs(f:GetDescendants()) do
            if c:IsA("Model") and (c.Name=="Generator" or c:GetAttribute("Progress")~=nil or c:FindFirstChild("Point1",true) or c:FindFirstChild("Progress",true)) then
                if not seen[c.Address] then checkgen(c) end
            end
        end
    end)
    ...
    if changed then
        local gList={}
        for a,e in pairs(state.gendata) do
            if type(e)~="table" then state.gendata[a]=nil; continue end
            local p
            pcall(function()
                if e.object and e.object:IsA("BasePart") then
                    p=e.object.Position
                elseif e.model then
        ...
        table.sort(gList,function(a,b) if mabs(a.px-b.px)<10 then return a.pz<b.pz end; return a.px<b.px end)
        for i,v in ipairs(gList) do
            if type(v.e)=="table" then
...
local function scanignorefolder()
    local f=gettrapfolder(); if not f then return end
    ...
    pcall(function()
        local function checkscan(c)
            ...
            if n=="trap" then if state.trapenabled then scantrap(c) end
            elseif n=="minion" then if state.trapenabled then scanminion(c) end
            elseif n=="battery" then if state.batteryenabled or state._forcescan then scanbattery(c) end end
        end
        for _,c in ipairs(f:GetChildren()) do
            checkscan(c)
            if c:IsA("Folder") or c:IsA("Model") then pcall(function() for _,sub in ipairs(c:GetChildren()) do checkscan(sub) end end) end
        end
    end)
    scanactive=false
end
...
local function watchkillerFolder()
    ...
    childvm:OnChildRemoved(kf,function(m)
        ...
        pcall(function() for _,c in ipairs(kf:GetChildren()) do if c:IsA("Model") then stillHas=true; break end end end)
...
local function teleporttoposition(hrp,pos,label)
    pcall(function()
        ...
        task.spawn(function()
            for i=1,5 do task.wait(0.05); if char and char:IsA("Model") then char:PivotTo(tgt) else hrp.CFrame=tgt end end
        end)
    end)
...
local function getbatterypos(hrpP)
    local f=gettrapfolder(); if not f or not hrpP then return nil end
    local bp,bd
    local function check(c)
        ...
        if n=="battery" then
            local p
            pcall(function()
                if c:IsA("BasePart") then p=c.Position
                else local part=c:FindFirstChildWhichIsA("BasePart",true); if part then p=part.Position end end
            end)
            if p then local d=(p-hrpP).Magnitude; if not bd or d<bd then bd=d; bp=p end end
        end
    end
    pcall(function() for _,c in ipairs(f:GetDescendants()) do check(c) end end)
    return bp,bd
end
...
local function teleporttogenerator()
    pcall(function()
        ...
        local hrp=c:FindFirstChild("HumanoidRootPart") or c:FindFirstChild("Torso"); if not hrp then return end
        local hrpP=hrp.Position; local mgp,mgd; local gf=getgenfolder()
        if gf then
            for _,c in ipairs(gf:GetDescendants()) do
                if c:IsA("Model") and (c.Name=="Generator" or c:GetAttribute("Progress")~=nil or c:FindFirstChild("Progress",true)) then
                    ...
                    if prog<100 then
                        ...
                        if p then local d=(p-hrpP).Magnitude; if not mgd or d<mgd then mgd=d; mgp=p end end
                    end
                end
            end
        end
...
local function teleporttofusebox()
    ...
    local hrp=char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Torso"); if not hrp then return end
    local fbf=getfuseboxfolder()
    if not fbf then pcall(function() notify("FuseBox TP","FuseBoxes folder not found",3) end); return end
    local bestPos,bestD
    local function trynode(fb)
        local inserted=false
        pcall(function() local v=fb:GetAttribute("Inserted"); if v==true then inserted=true end end)
        if inserted then return end
        local pos
        pcall(function()
            if fb:IsA("BasePart") then pos=fb.Position
            else local pp=fb.PrimaryPart or fb:FindFirstChildWhichIsA("BasePart",true); if pp then pos=pp.Position end end
        end)
        if pos then local d=(pos-hrp.Position).Magnitude; if not bestD or d<bestD then bestD=d; bestPos=pos end end
    end
    pcall(function()
        for _,fb in ipairs(fbf:GetDescendants()) do
            local n=fb.Name:lower()
            if n=="fusebox" or n=="fuse_box" or n=="fuse" then trynode(fb) end
        end
        if not bestPos then
            for _,fb in ipairs(fbf:GetChildren()) do trynode(fb) end
        end
    end)
...
local function watchignoreFolder()
    ...
    childvm:OnChildAdded(f,function(c)
        ...
        if n=="trap" then if state.trapenabled then scantrap(c) end
        elseif n=="minion" then if state.trapenabled then scanminion(c) end
        elseif n=="battery" then if state.batteryenabled then scanbattery(c) end end
    end)
    childvm:OnChildRemoved(f,function(c)
        local n=c.Name:lower()
        if n=="trap" then cleanuptrap(c)
        elseif n=="minion" then cleanupminion(c)
        elseif n=="battery" then cleanupbattery(c) end
    end)
end
...
local function updateentityesp(dr, en, col, bc)
    ...
    local vs; pcall(function() vs=camera.ViewportSize end)
    local clk=os.clock(); local v2=Vector2.new
    local gg,dg

    for a,e in pairs(dr) do
        local mdl,rt,ig=e.model,e.root,e.isGen
        if not mdl or not mdl.Parent then
            removeentity(e); dr[a]=nil
        elseif mdl==localplayer.Character and not state.optself then
            hideentity(e)
        else
            rt=getroot(mdl,ig) or rt; e.root=rt
            local rPos; if rt then pcall(function() rPos=rt.Position end) end
            if not rPos then
                hideentity(e)
            else
                local sp,on; pcall(function() sp,on=WorldToScreen(rPos) end)
                local onScreen=on and sp and vs and sp.X>-200 and sp.Y>-200 and sp.X<vs.X+200 and sp.Y<vs.Y+200
                ...
                elseif ig then
                    ...
                    if e.gp then
                        local pv=e.gp(); local dn=pv>=100
                        e.pg.Text=dn and (state.tagopen.."done"..state.tagclose) or (state.tagopen.."prog: "..tostring(pv).."%"..state.tagclose)
                        e.pg.Color=dn and dg or gg; if dn then e.nm.Color=dg end
                        e.pg.Position=v2(sp.X,ny+16); e.pg.Visible=true
                    end
                else
                    local nm,_=getname(mdl,false,e); e.nm.Text=nm
                    local ecol,ebc=col,bc
                    local d=0; if mrP then d=mfloor((rPos-mrP).Magnitude) end
                    local top,bot
                    pcall(function() top=rPos+offsettop; bot=rPos-offsetbottom end)
                    if not top then top=rPos end; if not bot then bot=rPos end
                    local ts,ton,bs,bon
                    pcall(function() ts,ton=WorldToScreen(top); bs,bon=WorldToScreen(bot) end)
                    local bh,bw=70,40
                    if ton and bon then bh=mabs(ts.Y-bs.Y); bw=bh/1.5 end
                    local x,y=sp.X-bw/2,sp.Y-bh/2
                    e._hid=nil
                    if ecol and e._col~=ecol then
                        e._col=ecol; e.nm.Color=ecol; e.tr.Color=ecol
                    ...
                    if state.opttracers and vs then
                        e.tr.From=v2(vs.X/2,vs.Y); e.tr.To=v2(sp.X,y+bh); e.tr.Visible=true
                    ...
                    if state.optskeleton then
                        ...
                        if hd and la and ra and ll and rl then
                            ...
                            pcall(function() hdP=hd.Position; laP=la.Position; raP=ra.Position; llP=ll.Position; rlP=rl.Position end)
                            if hdP and laP and raP and llP and rlP then
                                local hdSp,hdOn,laSp,laOn,raSp,raOn,llSp,llOn,rlSp,rlOn
                                pcall(function()
                                    hdSp,hdOn=WorldToScreen(hdP); laSp,laOn=WorldToScreen(laP)
                                    raSp,raOn=WorldToScreen(raP); llSp,llOn=WorldToScreen(llP); rlSp,rlOn=WorldToScreen(rlP)
                                end)
                                if hdOn and laOn and raOn and llOn and rlOn then
                                    ...
                                    local neckY=hdSp.Y+headRad+bh*0.02; local collarW=bw*0.35
                                    ...
                                    local hipW=bw*0.1; local lHipX,rHipX=cx-hipW,cx+hipW
                                    e.skPelvis.From=v2(lHipX,pelvisY); e.skPelvis.To=v2(rHipX,pelvisY); e.skPelvis.Visible=true
                                    local laClampX=mmin(slaX,cx-bw*0.05); local raClampX=mmax(sraX,cx+bw*0.05)
                                    local lElbowX=lShX+(laClampX-lShX)*0.5; local lElbowY=neckY+(slaY-neckY)*0.5
                                    local rElbowX=rShX+(raClampX-rShX)*0.5; local rElbowY=neckY+(sraY-neckY)*0.5
                                    ...
                                    local llClampX=mmin(sllX,cx-bw*0.02); local rlClampX=mmax(srlX,cx+bw*0.02)
                                    local lKneeX=lHipX+(llClampX-lHipX)*0.5; local lKneeY=pelvisY+(sllY-pelvisY)*0.5
                                    local rKneeX=rHipX+(rlClampX-rHipX)*0.5; local rKneeY=pelvisY+(srlY-pelvisY)*0.5
                    ...
                    if isKiller then
                        local charAttr="?"
                        pcall(function() local cv=mdl:GetAttribute("Character"); if type(cv)=="string" and cv~="" then charAttr=cv end end)
                        local hp=e.targetHp or 100
                        if e.humanoid then pcall(function() local h=e.humanoid.Health; if type(h)=="number" then hp=h end end) end
                        e.nm.Text=string.format("%s | %s | (%d)", nm, charAttr, mfloor(hp))
                    end
                    e.nm.Position=v2(sp.X,ny); e.nm.Visible=true
                    ...
                    local hp=e.targetHp or 100; local mhp=e.maxHp or 100
                    if e.humanoid then
                        pcall(function()
                            local h=e.humanoid.Health; local m=e.humanoid.MaxHealth
                    ...
                    e.targetHp=hp; if type(mhp)~="number" or mhp<=0 then mhp=100 end
                    local pct=mmax(0,mmin(hp/mhp,1)); e.hpLast=hp
                    e.hpSmooth=e.hpSmooth+((e.hpVis or 100)-e.hpSmooth)*0.08
                    e.hpVis=(e.hpVis or 100)+(pct*100-(e.hpVis or 100))*0.15
                    local hbW,hbH=4,bh; local hx=x-hbW-3
                    e.hpBg.Position=v2(hx-1,y-1); e.hpBg.Size=v2(hbW+2,hbH+2); e.hpBg.Visible=true
                    local dmgH=mfloor(hbH*(e.hpSmooth/100)); local fillH=mfloor(hbH*(e.hpVis/100))
                    if dmgH>fillH and dmgH>0 then e.hpDmg.Position=v2(hx,y+(hbH-dmgH)); e.hpDmg.Size=v2(hbW,dmgH); e.hpDmg.Visible=true
                    else e.hpDmg.Visible=false end
                    if fillH>0 then e.hpFill.Position=v2(hx,y+(hbH-fillH)); e.hpFill.Size=v2(hbW,fillH); e.hpFill.Visible=true
...
local function updateobjectesp(dr, en, c)
    ...
    for a,e in pairs(dr) do
        ...
        if not mdl or not mdl.Parent then removeobject(e); dr[a]=nil
        else
            if not rt or not rt.Parent then
                if e._noRoot then if e.nm then e.nm.Visible=false end; if e.dt then e.dt.Visible=false end
                else
                    pcall(function() rt=mdl:IsA("BasePart") and mdl or mdl:FindFirstChild("HumanoidRootPart") or mdl:FindFirstChild("Core") or mdl.PrimaryPart or mdl:FindFirstChildWhichIsA("BasePart",true) end)
                    if rt then e.root=rt else e._noRoot=true end
                end
            end
            local rPos; if e.root then pcall(function() rPos=e.root.Position end) end
            if not rPos then if e.nm then e.nm.Visible=false end; if e.dt then e.dt.Visible=false end
            else
                local sp,on; pcall(function() sp,on=WorldToScreen(rPos) end)
                local onScreen=on and sp and vs and sp.X>-200 and sp.Y>-200 and sp.X<vs.X+200 and sp.Y<vs.Y+200
                if onScreen then
...
UI.AddTab("Bite By Night",function(tab)
    ...
    opts:Toggle("tag_brackets","Use Brackets [ ]",function(s)
        if s then state.tagopen="["; state.tagclose="]"; espmod.tag_open="["; espmod.tag_close="]"
        else state.tagopen="<"; state.tagclose=">"; espmod.tag_open="<"; espmod.tag_close=">" end
    end)
...
connectsignal(runservice.RenderStepped,function()
    ...
    if state.steptick%2==0 then
        pcall(function()
            if localplayer.Character then
                state.myroot = localplayer.Character:FindFirstChild("Torso")
                    or localplayer.Character:FindFirstChild("HumanoidRootPart")
            end
        end)
    end

    if state.steptick >= state.esp_refresh_rate then
        state.steptick = 0
    ...
    if state.steptick%3 == 0 then
        state.dangercolor = nil; espmod.danger_color = nil
        local mrP; if state.myroot then pcall(function() mrP=state.myroot.Position end) end
        if mrP then
            local nd=math.huge
            for _,e in pairs(state.killerdata) do
                pcall(function()
                    if e.root and e.root.Position then
                        local dd=(e.root.Position-mrP).Magnitude
                        if dd<nd then nd=dd end
                    end
                end)
            end
            if nd<60 then
